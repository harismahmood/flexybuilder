...
<body id="builder">
<div class="loading">
    <div class="animated fadeIn infinite"><img src="{{ url('img/loading.png') }}" class="loading-image img-responsive"/>
    </div>
</div>
            ...
            <div id="container-canvas-center">
                <div id="columns-canvas-center">
                    ...
                    <div onmouseenter="showLabel('canvas-center','Main Content')" onmouseleave="removeLabel()" id="canvas-center">
                        {{--<img src="https://i.ytimg.com/vi/0ere0oloHh0/maxresdefault.jpg" class="img-responsive" />--}}
                    </div>
    ...
    {{  Session::get('script') }}
       ...
       $(document).ready(function () {
        ...
        function colorUpd(typeCC) {
            $.post('/ajax/colorupd', {color: typeCC, id:{{ Session::get('user_id') }}, _token: '{{ csrf_token() }}'});
        }

        $.post('/ajax/colorget', {id:{{  Session::get('user_id') }}, _token: '{{ csrf_token() }}'}, function (data) {
            //White
            if (data == 0) {
                $("body").removeAttr('id');
                $("body").attr('id', 'white');
